...
##
## WHY
## build_synthetic_inputs.R relabelled affiliation_group and affiliation_unit as
...
##
## Only affiliation_group feeds the compatibility score (as BG_mentor / BG_mentee, via the
## same-business-group preference). affiliation_unit is carried through the pipeline but
...
for (f in files) {
  ...
  for (cl in c("affiliation_group", "affiliation_unit", "display_name")) {
...
before <- bind_rows(lapply(frames, function(d) tibble(g = d$affiliation_group, u = d$affiliation_unit)))
...
for (f in names(frames)) {
  d <- frames[[f]]
  d$affiliation_group <- unname(group_of[d$display_name])
  d$affiliation_unit  <- unname(unit_of[d$display_name])
...
after <- bind_rows(lapply(list.files(opt$outdir, pattern = "^survey_synthetic_", full.names = TRUE),
                          function(p) {
                            ...
                            tibble(g = d$affiliation_group, u = d$affiliation_unit)
                          }))
